package com.czq.utils.redisutils.utils;
...
public class RedisUtils {
    ...
    public boolean expire(String key, long time) {
        try {
            if (time > 0) {
                redisTemplate.expire(key, time, TIME_UNIT);
            }
            return true;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /**
     * 向一张 Hash 表 中放入数据，并设置时间，如不存在则创建
     *
     * @param key   键
     * @param item  项
     * @param value 值
    ...
    public List<Object> lGet(String key) {
        try {
            final long START_INDEX = 0;
            final long END_INDEX = 0;
            return redisTemplate.opsForList().range(key, START_INDEX, END_INDEX);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
